""" Steps in Indessa Bank Project
1. Data Cleanig and removing the null Values- Column by Column
2. Balancing the Dataset- Considering undersampling as there are large number of records
3. EDA- In depth understanding of the data
4. Algorithm Implementation(All major ML algorithms have been Implemented and thier accuracy has been shared)
    a. Logistic Regression
    b. K- Nearest Neighbours
    c. Naive Bayes
    d. SVM
    e. Decision tree
    f. Ensemble Technique 1- Bagging- Random Forest
    g.Ensemble Technique 2- Boosting- XGBoosting
5. Testing it on Test Data
6. Submissions"""
...
df_train=pd.read_csv("E:\\interview Assignments\\ML_Artivatic_dataset\\Final\\Raw Data\\train_indessa.csv")
df_test=pd.read_csv("E:\\interview Assignments\\ML_Artivatic_dataset\\Final\\Raw Data\\test_indessa.csv")
...
# Step-2 Checking if the train Dataset is imbalanced or not 

df_train["loan_status"].value_counts()

# Defaulter percentage is 23.63% and Non - Defaulter % is 76.36 % if we train a model with this imbalanced data set there is high chance of getting type 2 error for test data. 
# Considering this we have to make the dataset balanced
# Since there are large number of records we can go for UnderSampling for the train data set
df_train_X=df_train.drop("loan_status",axis=1)
df_train_y=df_train["loan_status"]

# Library for up sampling - imblearn

from imblearn.under_sampling import NearMiss

us=NearMiss()

df_train_X,df_train_y=us.fit_sample(df_train_X,df_train_y)
df_train_y.value_counts()

# Dataset has been made balanced with Defaulters- 125827 & Non Defaulters- 125827

# Writing the Cleaned Balanced train Data Set into a csv File

train_bal=pd.concat([df_train_X,df_train_y],axis=1)

train_bal.to_csv("E:\\interview Assignments\\ML_Artivatic_dataset\\Final\\Cleaned Balanced Data Set\\train.csv")
df_test.to_csv("E:\\interview Assignments\\ML_Artivatic_dataset\\Final\\Cleaned Balanced Data Set\\test.csv")

# Step -3 EDA - Understanding Indessa Dataset in depth

# Hence for train data will be the balanced dataset called train
train=pd.read_csv("E:\\interview Assignments\\ML_Artivatic_dataset\\Final\\Cleaned Balanced Data Set\\train.csv")
test=pd.read_csv("E:\\interview Assignments\\ML_Artivatic_dataset\\Final\\Cleaned Balanced Data Set\\test.csv")


plt.boxplot(train["loan_amnt"],vert=False,patch_artist=True)
# Majority of the loan amount applied is between $7000-$20000

plt.boxplot(train["funded_amnt"],vert=False,patch_artist=True)

plt.boxplot(train["funded_amnt_inv"],vert=False,patch_artist=True)

plt.scatter(train["funded_amnt_inv"],train["term"])

# The loan tenure is of 2 types- 36 months and 60 months


# Doing a Bi-Variate analysis with respect to Loan_status(sns.pairplot image size too large)
sns.violinplot(train["int_rate"],train["term"],hue=train["loan_status"])

sns.FacetGrid(train["loan_amnt"],train["annual_inc"],hue=train["loan_status"])

sns.FacetGrid(train["dti"],train["annual_inc"],hue=train["loan_status"])

sns.FacetGrid(train["revol_bal"],train["annual_inc"],hue=train["loan_status"])

sns.FacetGrid(train["revol_bal"],train["revol_util"],hue=train["loan_status"])

sns.FacetGrid(train["tot_cur_bal"],train["total_rev_hi_lim"],hue=train["loan_status"])
...
print(auc(logistic_fpr,logistic_tpr))- #auc- 99.79%

print (accuracy_score(Y_test,Y_pred)) #auc-99.79%
...
print(auc(nb_fpr,nb_tpr))       #79.47%
print(accuracy_score(Y_test,Y_pred))    #79.47%
...
print(auc(sv_fpr,sv_tpr)) # 99.47
print(accuracy_score(Y_test,Y_pred)) # 99.47
...
print(auc(rf_fpr,rf_tpr)) #99.99 % 
print(accuracy_score(Y_test,Y_pred)) #99.99 % 
...
print(auc(xgb_fpr,xgb_tpr))- # 100%
print(accuracy_score(Y_test,Y_pred)) # 100% 

# All algorithms implemented and accuracy is 100% for DT and XgBoost
...
# In the test data 177476 are non defaulters and 177475 are defauters
df=df[0]
...
submission.to_csv("E:\\interview Assignments\\ML_Artivatic_dataset\\Final\\Cleaned Balanced Data Set\\final_submission.csv")
